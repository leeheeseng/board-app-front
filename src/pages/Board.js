...
const Board = () => {
    ...
    const findById = useCallback(async () => {
        try {
            const response = await axios.get(`http://223.130.163.57:9090/boards/${id}`, {
                headers: {
    ...
    const deleteById = useCallback(async () => {
        try {
            const resonse = await axios.delete(`http://223.130.163.57:9090/boards/${id}`, {
                headers: {
                    Authorization: `Bearer ${sessionStorage.getItem('ACCESS_TOKEN')}`
                }
            });
    ...
    const modify = useCallback(async (formData) => {
        try {
            const response = await axios.patch('http://223.130.163.57:9090/boards', formData, {
                headers: {
                    Authorization: `Bearer ${sessionStorage.getItem('ACCESS_TOKEN')}`
                }
            });
                            ...
                            >
                                ...
                                <img width='150px'
                                     ...
                                     src={boardFile.filetype === 'image'
                                        ? `https://kr.object.ncloudstorage.com/bitcamp122/${boardFile.filepath}${boardFile.filename}`
